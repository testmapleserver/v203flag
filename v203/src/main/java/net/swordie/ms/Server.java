...
public class Server extends Properties {
	...
	private void init(String[] args) {
		...
		DatabaseManager.init();
                log.info("fnished initing database");
		...
		try {
			...
			e.printStackTrace();
                ...
                log.info("loaded wz data");
		StringData.load();
                log.info("loaded string data");
		FieldData.loadWorldMap();
                log.info("loaded world map");
		...
		log.info(String.format("Finished loading serverrrrr in %dms", System.currentTimeMillis() - startNow));
		new Thread(() -> {
	...
	private void checkAndCreateDat() {
		// changed to fields because equips isn't a thing??
