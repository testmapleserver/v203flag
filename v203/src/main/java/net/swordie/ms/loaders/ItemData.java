...
public class ItemData {
                            ...
                            for (Node rewardInfoNode : XMLApi.getAllChildren(rewardNode)) {
                                ...
                                switch (name) {
                                    case "count":
                                        ...
                                        break;
                                    case "item":
                                        log.info("item value is: ");
                                        log.info(value);
                                        log.info("value.trim() is: ");
                                        log.info(value.trim());
                                        iri.setItemID(Integer.parseInt(value.trim()));
                                        break;
                                    case "prob":
    ...
    public static void generateDatFiles() {
        ...
        loadEquipsFromWz();
        log.info("finished loading equips from wz");
        loadMountItemsFromFile();
        log.info("finished omuntain items from file");
        loadItemsFromWZ();
        log.info("finished loading items from wz");
        loadPetsFromWZ();
        log.info("finishe dloading pets from wz");
        loadItemOptionsFromWZ();
        log.info("finished loading item options from wz");
        QuestData.linkItemData();
        saveEquips(ServerConstants.DAT_DIR + "/equips");
        log.info("finished saving equips to /equips");
        saveItems(ServerConstants.DAT_DIR + "/items");
        log.info("finished saving items to /items");
        savePets(ServerConstants.DAT_DIR + "/pets");
        log.info("finished saving pets to /pets");
        saveItemOptions(ServerConstants.DAT_DIR);
