...
public class LoginHandler {
    ...
    public static void handleCheckLoginAuthInfo(Client c, InPacket inPacket) {
        log.info("handling check login auth info");
    ...
    public static void handleSelectWorld(Client c, InPacket inPacket) {
        log.info("handling select world");
        inPacket.decodeByte();
...
//        int userId = inPacket.decodeInt();
        ...
        log.info(pic);
        log.info("acc pic is: ");
        log.info(c.getAccount().getPic());
        log.info(pic == c.getAccount().getPic());
        log.info(pic.equals(c.getAccount().getPic()));
        //if (BCrypt.checkpw(pic, c.getAccount().getPic())) {
        if(pic.equals(c.getAccount().getPic())){
            log.info("correct pic");
            success = true;
        } else {
            log.info("wrong pic");
